<template>
                ...
                <td class="title" :colspan="isMobile ? 3 : 1" scope="row">
                  <div class="notice-title">
                    <router-link
                      ...
                      :to="{ name: 'NoticeDetail', params: { id: String(row.id) } }"
                      class="notice-title__text"
                    >
                      {{ row.title }}
                    </router-link>
  ...
  <style scoped>
  /* router-link는 <a>로 렌더 — 목록에서 링크처럼 보이게 */
  .notice-title :deep(a.notice-title__text) {
    color: inherit;
    text-decoration: none;
    cursor: pointer;
  }
  .notice-title :deep(a.notice-title__text:hover) {
