<template>
  <section class="page main-page">

    <div class="main-cont banner">


      <div class="banner-wrapper">
        <ul>
          <li v-for="(item, idx) in banners" :key="idx">
            <img :src="$getImage(item.img)" :alt="item.alt">
          </li>
        </ul>

        <img src="/src/assets/images/main_img_01.png" alt="메인 배너 이미지">
        <img src="/src/assets/images/main_img_02.png" alt="메인 배너 이미지">
      </div>

      <div class="banner-pagination">
        <span></span>
      </div>
    </div>

    <div class="main-cont business">
      <h2 class="main-title">사업영역</h2>

      <CardHover />
    </div>

    <div class="main-cont job">
      <h2 class="main-title">채용공고</h2>

      <div class="job-img">
        <a href="https://career.mgdatasystem.co.kr/" target="_blank">
          <img src="/src/assets/images/main_banner_01.png" alt="채용공고 이미지">
        </a>
      </div>

    </div>

    <div class="main-cont notice">
      <h2 class="main-title">공지사항</h2>

      <CardSlider />

    </div>

    <!-- 메인 배너, 카드, 섹션 등 -->

    <!-- Top button -->
    <BaseTopButton/>
  </section>
</template>

<script setup>
import { ref } from 'vue';

const banners = ref([
  {img: 'main_img_01.png', alt: '메인 배너 이미지1'},
  {img: 'main_img_02.png', alt: '메인 배너 이미지2'},
])
</script>
  
<style scoped>
@use '@scss/pages/_main.scss';
</style>