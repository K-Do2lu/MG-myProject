                        ...
                        <tbody>
                            <tr v-for="row in rows" :key="row.id">
                                <td class="title" scope="row">
                                    <a href="#" class="notice-title__link" @click.prevent>
                                        {{ row.title }}
                                    </a>
                                
                                    <!-- 뱃지 (중요 공지 등) -->
                                    <span v-if="row.isImportant"  class="notice-title__badge"role="status" aria-label="중요 게시물">HOT</class=>
                                    <!-- 첨부파일 아이콘 -->
...
<script setup>
    const rows = [
        ...
        {id: 9, title: '정규직(일반직) 및 계약직 상시채용', isImportant: true, hasAttachment: true, date: '2026-02-23', views: 625},
    ]
