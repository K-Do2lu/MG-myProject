<template>
  ...
  // 페이지 번호에 따라 트랙을 왼쪽으로 0%, 100%, 200% 만큼 이동
  const trackStyle = computed(() => {
  const x = currentPage.value * 100
  return {
    transform: `translateX(-${x}%)`,
    transition: noTransition.value ? 'none' : 'transform .4s ease',
  }
})
  ...
  // 마지막 페이지에서 한 번 더 next: 
  // 1) 끝까지 슬라이드 (마지막 애니메이션)
  currentPage.value += 1
  // 2) 애니메이션 끝난 뒤에 transition 끄고 0으로 점프 → 다시 켬
  setTimeout(() => {
    noTransition.value = true      // .card-slider__track 에서 transition 제거
    currentPage.value = 0          // 눈에 안 띄게 원위치
    void document.body.offsetHeight // reflow 강제 (옵션)
    noTransition.value = false     // 다시 transition 활성화
  }, 400) // transition 시간(.4s)과 맞추기
  }
  ...
  /* 자동 슬라이드(autoplay) */
  const autoplayDelay = 4000 // 4초마다 이동 (원하는 값으로 조정)
