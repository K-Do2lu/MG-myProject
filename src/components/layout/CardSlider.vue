...
const slideStyle = computed(() => ({
  transform: `translateX(-${currentIndex.value * (100 / viewCount.value)}%)`,
  transition: 'transform 0.5s ease-in-out',
}));
// 다음 버튼
const nextSlide = () => {
  if (currentIndex.value < totalItems - viewCount.value) {
    currentIndex.value++;
  } else {
...
const prevSlide = () => {
  if (currentIndex.value > 0) {
    ...
    currentIndex.value = totalItems - viewCount.value;
  }
};
